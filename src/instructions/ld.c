...
/**
 * @file ld.c
 * @brief The file containing the ld function
 */

#include "../../include/corewar.h"

/**
 * @brief Execute the direct ld function
 * @param champion The champion to execute the direct ld function
 * @param address The address of the direct ld function
 * @return <b>void</b>
 */
static void exec_direct_ld(champion_t *champion, node_t *address)
{
...
/**
 * @brief Execute the indirect ld function
 * @param champion The champion to execute the indirect ld function
 * @param address The address of the indirect ld function
 * @return <b>void</b>
 */
static void exec_indirect_ld(champion_t *champion, node_t *address)
{
    ...
    champion->reg[register_choose - 1] = read_int_in_address(
        get_arena_node(champion->address + index % IDX_MOD), 4);
    update_carry(champion, champion->reg[register_choose - 1]);
}
...
void exec_ld(champion_t *champion, node_t *address)
{
    ...
    && get_code_byte_index(coding_byte, 1) == REG_CODE)
        exec_direct_ld(champion, address);
    if (get_code_byte_index(coding_byte, 0) == IND_CODE
    && get_code_byte_index(coding_byte, 1) == REG_CODE)
        exec_indirect_ld(champion, address);
